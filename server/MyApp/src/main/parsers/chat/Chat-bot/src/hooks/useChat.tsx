...
import { 
  ...
  CreateConversationRequest, StreamChunk, StreamEventType,
...
export const ChatProvider: React.FC<ChatProviderProps> = ({ children }) => {
  ...
  const updateConversation = useCallback(async (
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        throw new Error(response.message || 'Ошибка обновления диалога');
      }
    } catch (err: any) {
  ...
  // Обработка chunk'ов из SSE потока
  const handleStreamChunk = useCallback((chunk: StreamChunk) => {
    if (chunk.message_id) {
      setMessages(prev => 
        prev.map(msg => {
          if (msg.id === chunk.message_id) {
            return {
              ...msg,
              content: msg.content + (chunk.chunk || ''),
              complete: chunk.done || false,
            };
          }
          return msg;
        })
      ...
      // Создаем финальное сообщение когда поток завершен
