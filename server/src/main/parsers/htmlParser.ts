...
export const parseDynamicContent = async (
  ...
  try {
    ...
    try {
      ...
      // Извлечение данных
      const results = await page.$$eval(selector, (elements: any[], opts: ParseOptions) => {
        const { attribute, returnHtml } = opts as ParseOptions;
        
        return elements.map((el: { getAttribute: (arg0: string) => any; outerHTML: any; textContent: string; }) => {
          if (attribute) {
      ...
      const filteredResults = results.filter((item: { trim: () => { (): any; new(): any; length: number; }; }) => item.trim().length > 0);
