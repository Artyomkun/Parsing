...
import * as yaml from 'js-yaml'; // Добавлен импорт yaml
import * as xml2js from 'xml2js'; // Добавлен импорт xml2js
  ...
  try {
    switch (type) {
      ...
      case 'json2csv': {
        try {
          const parsedJson = JSON.parse(data);
          return Papa.unparse(parsedJson) || 'No content found'; // Исправлено
        } catch (error) {
          throw new Error('Invalid JSON format');
        }
      }
      ...
      case 'yaml2json': {
        try {
          ...
          return Promise.reject(`Invalid YAML format: ${errMsg}`);
        }
      }
