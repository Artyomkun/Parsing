...
export interface ParsedElement {
  text: string;
  html?: string;
  [key: string]: any; // Для динамических атрибутов
}
...
export const parseDynamic = async ({
  url,
  selector,
  attribute,
  returnHtml = false
}: ParseOptions): Promise<ParsedElement[]> => {
    ...
    for (const el of elements) {
      ...
      if (attribute) {
        text = (await el.evaluate(
          (node: Element, attr: string) => (node as HTMLElement).getAttribute(attr) || '', 
          attribute
        )).trim();
      } else {
        text = (await el.evaluate((node: Element) => (node as HTMLElement).textContent || '')).trim();
      }
      if (returnHtml) {
        html = await el.evaluate((node: Element) => (node as HTMLElement).outerHTML) as string;
      }
