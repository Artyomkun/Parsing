...
const log = (msg: string) => console.log(`[Parser] ${msg}`);
export const parseDynamicContent = async (
  ...
  try {
    ...
    const attribute = options && (options as any).attribute;
    const returnHtml = options && (options as any).returnHtml;
    const evalResults = await page.$$eval(selector, (elements: any[], attr: any, returnHtml: any) =>
      elements.map((el: { textContent: string; outerHTML: string; getAttribute: (arg0: any) => any; }) => {
    ...
    const filtered = evalResults.filter((r: { text: string | any[]; }) => r.text.length > 0 || (attribute && [attribute]));
    if (filtered.length === 0) {
      throw new Error(`Найдено ${evalResults.length}, но все пустые или без атрибута`);
    }
      return filtered.map((item: any) => item.text);
    } catch (error) {
      throw new Error(`Ошибка при динамическом парсинге: ${(error as Error).message}`);
    } finally {
